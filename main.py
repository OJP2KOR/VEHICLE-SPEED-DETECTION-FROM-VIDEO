import cv2
import numpy as np
import time
import copy
import os							# "https://www.geeksforgeeks.org/os-module-python-examples/"
import glob							# "https://docs.python.org/3/library/glob.html"

from datetime import datetime

from tracker import Tracker

if __name__ == '__main__':
	FPS = 30
	'''
		Distance to line in road: ~0.025 km
	'''
	ROAD_DIST_KM = 0.025

	HIGHWAY_SPEED_LIMIT = 120

	# Initial background subtractor and text font
	fgbg = cv2.createBackgroundSubtractorMOG2()			# "https://www.slideshare.net/ravi5raj_88/background-subtraction"
	font = cv2.FONT_HERSHEY_PLAIN

	centers = [] 

	# y-cooridinate for speed detection line
	Y_THRESH = 200

	blob_min_width_far = 6	# A Blob is a group of connected pixels in an image that share some common property ( E.g grayscale value )
	blob_min_height_far = 6

	blob_min_width_near = 18
	blob_min_height_near = 18

	frame_start_time = None

	# Create object tracker
	tracker = Tracker(80, 3, 2, 1)

	cap = cv2.VideoCapture ('C:/Users/user/PycharmProjects/speed-detector/sample3.mp4')

	while True:
		centers = []
		frame_start_time = datetime.utcnow()	#Return the current UTC date and time, with tzinfo None. This is like now(), but returns the current UTC date and time, as a naive datetime object.
		ret, frame = cap.read()					#Grabs, decodes and returns the next video frame

		orig_frame = copy.copy(frame)

		#  Draw line used for speed detection
		cv2.line(frame,(0, Y_THRESH),(640, Y_THRESH),(255,0,0),2)	#cv2.line(img, pt1, pt2, color[, thickness[, lineType[, shift]]]) → None


		# Convert frame to grayscale and perform background subtraction
		gray = cv2.cvtColor (frame, cv2.COLOR_BGR2GRAY)
		fgmask = fgbg.apply (gray)  # https://docs.opencv.org/3.4/db/d5c/tutorial_py_bg_subtraction.html

		# Perform some Morphological operations to remove noise
		kernel = np.ones((4,4),np.uint8)    # https://docs.scipy.org/doc/numpy/reference/generated/numpy.ones.html
		kernel_dilate = np.ones((5,5),np.uint8) # https://docs.opencv.org/trunk/d9/d61/tutorial_py_morphological_ops.html
		opening = cv2.morphologyEx(fgmask, cv2.MORPH_OPEN, kernel)
		dilation = cv2.morphologyEx(opening, cv2.MORPH_CLOSE, kernel_dilate)

		contours, hierarchy = cv2.findContours(dilation, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) # https://docs.opencv.org/3.1.0/d4/d73/tutorial_py_contours_begin.html

		# Find centers of all detected objects
		for cnt in contours:
			x, y, w, h = cv2.boundingRect(cnt)  # Let (x,y) be the top-left coordinate of the rectangle and (w,h) be its width and height.

			if y > Y_THRESH:
				if w >= blob_min_width_near and h >= blob_min_height_near:
					center = np.array ([[x+w/2], [y+h/2]])
					centers.append(np.round(center))

					cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 0, 255), 2)
			else:
				if w >= blob_min_width_far and h >= blob_min_height_far:
					center = np.array ([[x+w/2], [y+h/2]])
					centers.append(np.round(center))

					cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)

		if centers:
			tracker.update(centers)

			for vehicle in tracker.tracks:
				if len(vehicle.trace) > 1:
					for j in range(len(vehicle.trace)-1):
                        # Draw trace line
						x1 = vehicle.trace[j][0][0]
						y1 = vehicle.trace[j][1][0]
						x2 = vehicle.trace[j+1][0][0]
						y2 = vehicle.trace[j+1][1][0]

						cv2.line(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 255), 2)

					try:
						'''
							TODO: account for load lag
						'''

						trace_i = len(vehicle.trace) - 1

						trace_x = vehicle.trace[trace_i][0][0]
						trace_y = vehicle.trace[trace_i][1][0]

						# Check if tracked object has reached the speed detection line
						if trace_y <= Y_THRESH + 5 and trace_y >= Y_THRESH - 5 and not vehicle.passed:
							cv2.putText(frame, 'I PASSED!', (int(trace_x), int(trace_y)), font, 1, (0, 255, 255), 1, cv2.LINE_AA)
							vehicle.passed = True

							load_lag = (datetime.utcnow() - frame_start_time).total_seconds()

							time_dur = (datetime.utcnow() - vehicle.start_time).total_seconds() - load_lag

							time_dur /= 3600

							
							vehicle.kph = ROAD_DIST_KM / time_dur

							# If calculated speed exceeds speed limit, save an image of speeding car
							if vehicle.kph > HIGHWAY_SPEED_LIMIT:
								print ('UH OH, SPEEDING!')
								cv2.circle(orig_frame, (int(trace_x), int(trace_y)), 20, (0, 0, 255), 2)
								cv2.putText(orig_frame, 'KPH: %s' % int(vehicle.kph), (int(trace_x), int(trace_y)),
											font, 1, (0, 0, 255), 1, cv2.LINE_AA)
								cv2.imwrite('speeding_%s.png' % vehicle.track_id, orig_frame)
								print ('FILE SAVED! \n')

						if vehicle.passed:
							# Display speed if available
							cv2.putText(frame, 'KPH: %s' % int(vehicle.kph), (int(trace_x), int(trace_y)), font, 1, (0, 255, 255), 1, cv2.LINE_AA)
						else:
							# Otherwise, just show tracking id
							cv2.putText(frame, 'ID: '+ str(vehicle.track_id), (int(trace_x), int(trace_y)), font, 1, (255, 255, 255), 1, cv2.LINE_AA)
					except:
						pass


		# Display all images
		cv2.imshow ('original', frame)
		cv2.imshow ('opening/dilation', dilation)
		cv2.imshow ('background subtraction', fgmask)

		# Quit when escape key pressed
		if cv2.waitKey(5) == 27:
			break

		# Sleep to keep video speed consistent
		time.sleep(1.0 / FPS)

	# Clean up
	cap.release()
	cv2.destroyAllWindows()

	# remove all speeding_*.png images created in runtime
	for file in glob.glob('speeding_*.png'):
		os.remove(file)